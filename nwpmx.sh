#!/bin/bash
# https://github.com/Weilbyte/nwpmx
...
function checkSupported {   
    ...
    if [ -z "$SUPPORTED" ]; then 
        ...
        if ! (printf '%s\n' "${SUPPORTEDARR[@]}" | grep -q -P "$PVEVersionMajor"); then
            ...
            echo -e "If you do run into any issues on >newer< versions, please consider opening an issue at https://github.com/Weilbyte/nwpmx/issues.${REG}"
        fi
    fi
}
...
function usage {
    if [ "$_silent" = false ]; then
        ...
        echo -e "Manages the nwpmx theme."
    ...
    #    echo -e "  utility-update       Update this utility\n" (to be implemented)
        ...
        echo -e "Report issues at: <https://github.com/Weilbyte/nwpmx/issues>"
    fi
}
...
function install {
    if isInstalled; then
        ...
        if [ "$OFFLINE" = false ]; then
            curl -s $BASE_URL/nwpmx/sass/nwpmx.css > /usr/share/pve-manager/css/dd_style.css
        else
            cp "$OFFLINEDIR/nwpmx/sass/nwpmx.css" /usr/share/pve-manager/css/dd_style.css
        fi

        if [ "$_silent" = false ]; then echo -e "${CHECKMARK} Downloading patcher"; fi
        if [ "$OFFLINE" = false ]; then
            curl -s $BASE_URL/nwpmx/js/nwpmx.js > /usr/share/pve-manager/js/dd_patcher.js
        else
            cp "$OFFLINEDIR/nwpmx/js/nwpmx.js" /usr/share/pve-manager/js/dd_patcher.js
        fi
        ...
        do
                if [ "$OFFLINE" = false ]; then
                    curl -s $BASE_URL/nwpmx/images/$image > /usr/share/pve-manager/images/$image
                else
                    cp "$OFFLINEDIR/nwpmx/images/$image" /usr/share/pve-manager/images/$image
                fi
                ((ITER++))
